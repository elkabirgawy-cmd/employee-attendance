/*
  # Debug Countdown Function
  
  Adding RAISE NOTICE statements to debug why cancellation isn't working
*/
...
BEGIN
  ...
  RAISE LOG 'DEBUG: v_is_problem = %, p_in_branch = %, p_gps_ok = %', v_is_problem, p_in_branch, p_gps_ok;
  ...
  RAISE LOG 'DEBUG: v_existing_pending IS %', CASE WHEN v_existing_pending IS NULL THEN 'NULL' ELSE 'NOT NULL (id=' || v_existing_pending.id || ')' END;
  ...
  RAISE LOG 'DEBUG: v_has_cancelled_entries = %', v_has_cancelled_entries;

  -- If no problem now, cancel any existing pending checkout
  IF NOT v_is_problem THEN
    RAISE LOG 'DEBUG: Entered NOT v_is_problem block';
    
    IF v_existing_pending IS NOT NULL THEN
      RAISE LOG 'DEBUG: Cancelling pending with id = %', v_existing_pending.id;
      ...
      RAISE LOG 'DEBUG: Successfully cancelled pending';
    ...
    RAISE LOG 'DEBUG: Returning OK status (no pending to cancel)';
  ...
  -- There IS a problem
  RAISE LOG 'DEBUG: There IS a problem';
  ...
  IF v_has_cancelled_entries AND v_existing_pending IS NOT NULL THEN
    RAISE LOG 'DEBUG: Deleting old pending and cancelled entries for fresh start';
  ...
  IF v_existing_pending IS NULL THEN
    ...
    RAISE LOG 'DEBUG: Creating new pending entry with ends_at = %', v_ends_at;
