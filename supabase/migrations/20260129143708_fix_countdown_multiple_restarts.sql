/*
  # Fix Countdown Multiple Restarts Within Same Session

  ## Problem
  Countdown only starts ONCE per check-in session. After recovery, it won't restart on new violations.

  ## Root Cause
  The `first_location_disabled_detected_at` field on attendance_logs was not being cleared on recovery,
  preventing countdown from restarting on subsequent violations.

  ## Solution
  1. Clear violation tracking fields on recovery
  2. Ensure new pending can be created even if old CANCELLED/DONE rows exist
  3. Only check for active PENDING rows when deciding to create new countdown

  ## Changes
  - On recovery: Clear first_location_disabled_detected_at on attendance_logs
  - On new violation: Only check for PENDING status (ignore CANCELLED/DONE)
  - Each violation gets fresh countdown from full duration
*/
