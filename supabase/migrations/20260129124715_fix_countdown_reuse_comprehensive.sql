/*
  ...
  ## السبب الجذري
  كانت دالة employee-heartbeat تستدعي `upsert_employee_heartbeat` القديمة
  بدلاً من `record_heartbeat_and_check_auto_checkout` الجديدة التي تحتوي على منطق
  حذف السجلات القديمة.

  ## الحل
  1. تحديث employee-heartbeat لاستخدام الدالة الصحيحة
  2. حذف الدالة القديمة التي لم تعد مستخدمة
*/

-- Drop the old unused function
DROP FUNCTION IF EXISTS upsert_employee_heartbeat(uuid, uuid, uuid, boolean, boolean, text);

-- Ensure the correct function exists with proper logic
