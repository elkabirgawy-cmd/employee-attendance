...
export async function detectReportCapabilities(): Promise<ReportCapabilities> {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select(`
        check_in_latitude,
        check_in_longitude,
        device:devices(device_name, device_model, os_type),
        shift:shifts(start_time, grace_period_minutes)
      `)
      .limit(1)
      .maybeSingle();
