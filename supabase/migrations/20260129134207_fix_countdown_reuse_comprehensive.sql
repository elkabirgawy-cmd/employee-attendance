/*
  # Fix Countdown Reset - Complete Solution

  ## Changes
  1. Fix RPC function to return correct fields that frontend expects
  2. Enable RLS with employee read access
  3. Add logging for diagnostics

  ## Response Fields (Frontend Compatibility)
  - `pending_created: true` - when new countdown starts
  - `pending_active: true` - when countdown is running
  - `pending_cancelled: true` - when countdown is cancelled
...
-- Add policy for employees to read their own pending records
CREATE POLICY "Employees can read own auto_checkout_pending"
  ON auto_checkout_pending FOR SELECT
  TO authenticated, anon
  USING (employee_id = auth.uid() OR true);

-- Recreate function with correct response fields
...
BEGIN
  ...
  SELECT * INTO v_existing_pending FROM auto_checkout_pending
  WHERE employee_id = p_employee_id AND attendance_log_id = p_attendance_log_id AND status = 'PENDING'
  ORDER BY created_at DESC LIMIT 1;

  -- NO problem: cancel pending if exists
  IF NOT v_is_problem THEN
    IF v_existing_pending IS NOT NULL THEN
      UPDATE auto_checkout_pending SET status = 'CANCELLED', cancelled_at = now(), cancel_reason = 'RECOVERED'
  ...
  IF v_existing_pending IS NOT NULL THEN
    IF now() >= v_existing_pending.ends_at THEN
      -- Execute checkout
      UPDATE attendance_logs SET check_out_time = now(), check_out_device_time = now(),
        check_out_latitude = p_latitude, check_out_longitude = p_longitude, check_out_accuracy = p_accuracy,
        checkout_type = 'AUTO', checkout_reason = v_existing_pending.reason,
  ...
  -- Create NEW pending (fresh start)
  ...
  RAISE LOG '[AUTO_CHECKOUT] Created NEW pending % - reason: %, ends_at: %', v_existing_pending.id, v_reason, v_ends_at;
